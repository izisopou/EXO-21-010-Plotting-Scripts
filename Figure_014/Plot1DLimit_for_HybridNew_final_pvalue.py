...
def setstyle():
    ...
    rt.gStyle.SetPadRightMargin(0.14) #0.065
    rt.gStyle.SetPadBottomMargin(0.15)
    rt.gStyle.SetPadLeftMargin(0.14) #0.17
...
if __name__ == '__main__':
    ...
    if options.doSignificance:
        h_limit.SetTitle(" ;Four-jet resonance mass [GeV];Local #it{p}-value")
    else:
...
#                  expectedLimit_minus2sigma_new[i] = expectedLimit_minus2sigma[(Box,model)][i]
    ...
    if options.doSignificance:
        ...
        h_limit.SetMinimum(0.000005) #0.00001
    else:
        if 'PF' in Box:
    ...
    for Box in Boxes:
        for model in models:    
            if options.doSignificance:
                ...
                gr_observedLimit[(Box,model)].SetMarkerSize(1)
                gr_observedLimit[(Box,model)].SetLineColor(rt.kBlack) #kRed
                gr_observedLimit[(Box,model)].SetMarkerColor(rt.kBlack) #kBlue
	...
	#l.DrawLatex(0.18,0.83,"CMS #scale[0.7]{#it{#bf{Preliminary}}}") #for PAS - ilias
        l.DrawLatex(0.18,0.82,"CMS") #for PAPER - ilias
    ...
    l.DrawLatex(0.56,0.92,"%.0f fb^{-1} (13 TeV)"%(options.lumi))
    ...
    if options.doSignificance:
        #lsig.DrawLatex(0.20,0.77,"m_{#chi}/M_{S} = %.2f, y_{uu}=0.4, y_{#chi}=0.6"%(alphatrue))
            #lsig.DrawLatex(0.55,0.85,"Y#rightarrow X X #rightarrow (jj) (jj)") #0.2, 0.77
            #lsig.DrawLatex(0.55,0.81,"M(X) / M(Y) = %.2f"%(alphatrue)) #0.2, 0.74

            lsig.DrawLatex(0.51,0.85,"Y#rightarrow X X #rightarrow (jj) (jj)") #FR
            lsig.DrawLatex(0.51,0.81,"M(X) / M(Y) = %.2f"%(alphatrue)) #FR
	...
	#Draw 1,2,3,4 sigma lines of local significance 
	l0sigma = rt.TLine(2000, 0.5, 10000, 0.5)
	l1sigma_v1 = rt.TLine(2000, 0.15865525, 2550, 0.15865525)
	l1sigma_v2 = rt.TLine(3000, 0.15865525, 10000, 0.15865525)
	l2sigma_v1 = rt.TLine(2000, 0.022750132, 2550, 0.022750132)
	l2sigma_v2 = rt.TLine(3000, 0.022750132, 10000, 0.022750132)
	l3sigma_v1 = rt.TLine(2000, 0.0013498980, 2550, 0.0013498980)
	l3sigma_v2 = rt.TLine(3000, 0.0013498980, 10000, 0.0013498980)
	l4sigma_v1 = rt.TLine(2000, 3.1671242e-05, 2550, 3.1671242e-05)
	l4sigma_v2 = rt.TLine(3000, 3.1671242e-05, 10000, 3.1671242e-05)

	l0sigma.SetLineStyle(rt.kDashed)
	l1sigma_v1.SetLineStyle(rt.kDashed)
	l1sigma_v2.SetLineStyle(rt.kDashed)
	l2sigma_v1.SetLineStyle(rt.kDashed)
	l2sigma_v2.SetLineStyle(rt.kDashed)
	l3sigma_v1.SetLineStyle(rt.kDashed)
	l3sigma_v2.SetLineStyle(rt.kDashed)
	l4sigma_v1.SetLineStyle(rt.kDashed)
	l4sigma_v2.SetLineStyle(rt.kDashed)
	l0sigma.SetLineWidth(2)
	l1sigma_v1.SetLineWidth(2)
	l1sigma_v2.SetLineWidth(2)
	l2sigma_v1.SetLineWidth(2)
	l2sigma_v2.SetLineWidth(2)
	l3sigma_v1.SetLineWidth(2)
	l3sigma_v2.SetLineWidth(2)
	l4sigma_v1.SetLineWidth(2)
	l4sigma_v2.SetLineWidth(2)
	l0sigma.Draw("same")
	l1sigma_v1.Draw("same")
	l1sigma_v2.Draw("same")
	l2sigma_v1.Draw("same")
	l2sigma_v2.Draw("same")
	l3sigma_v1.Draw("same")
	l3sigma_v2.Draw("same")
	l4sigma_v1.Draw("same")
	l4sigma_v2.Draw("same")


	lsigma_leg = rt.TLatex()
        lsigma_leg.SetTextAlign(22)
        lsigma_leg.SetTextSize(0.05)
        lsigma_leg.SetTextFont(42)
        lsigma_leg.SetTextSize(0.03)

	lsigma_leg.DrawLatex(2800, 0.15865525, "1#sigma")
	lsigma_leg.DrawLatex(2800, 0.022750132, "2#sigma")
	lsigma_leg.DrawLatex(2800, 0.0013498980, "3#sigma")
	lsigma_leg.DrawLatex(2800, 3.1671242e-05, "4#sigma")

	#Draw 1,2 sigma lines of global significance
	l1sigma_global_v1 = rt.TLine(2000, 1.52e-4, 7250, 1.52e-4)
	l1sigma_global_v2 = rt.TLine(7750, 1.52e-4, 10000, 1.52e-4)
	l2sigma_global_v1 = rt.TLine(2000, 2.02e-05, 7250, 2.02e-05)
	l2sigma_global_v2 = rt.TLine(7750, 2.02e-05, 10000, 2.02e-05)
	l1sigma_global_v1.SetLineStyle(7)
	l1sigma_global_v2.SetLineStyle(7)
	l2sigma_global_v1.SetLineStyle(7)
	l2sigma_global_v2.SetLineStyle(7)
	l1sigma_global_v1.SetLineColor(rt.kBlue)
	l1sigma_global_v2.SetLineColor(rt.kBlue)
	l2sigma_global_v1.SetLineColor(rt.kBlue)
	l2sigma_global_v2.SetLineColor(rt.kBlue)
	l1sigma_global_v1.SetLineWidth(2)
	l1sigma_global_v2.SetLineWidth(2)
	l2sigma_global_v1.SetLineWidth(2)
	l2sigma_global_v2.SetLineWidth(2)
	l1sigma_global_v1.Draw("same")
	l1sigma_global_v2.Draw("same")
	l2sigma_global_v1.Draw("same")
	l2sigma_global_v2.Draw("same")

	lsigma_global_leg = rt.TLatex()
        lsigma_global_leg.SetTextAlign(22)
        lsigma_global_leg.SetTextSize(0.05)
        lsigma_global_leg.SetTextFont(42)
        lsigma_global_leg.SetTextSize(0.03)
	lsigma_global_leg.SetTextColor(rt.kBlue)

	lsigma_global_leg.DrawLatex(7500, 1.52e-4, "1#sigma")
	lsigma_global_leg.DrawLatex(7500, 2.02e-05, "2#sigma")


	#Draw right axis for global pvalue
	rightaxis_title = rt.TLatex()
	rightaxis_title.SetTextAlign(22)
        rightaxis_title.SetTextSize(0.05)
        rightaxis_title.SetTextFont(42)
        rightaxis_title.SetTextSize(0.05)
	rightaxis_title.SetTextAngle(90)
	rightaxis_title.SetTextColor(rt.kBlue)

	rightaxis_title.DrawLatex(11200, 0.3, "Global #it{p}-value") #11200,0.4 for "Global p-value"
	

	rightaxis = rt.TLatex()
	rightaxis.SetTextAlign(22)
        rightaxis.SetTextSize(0.05)
        rightaxis.SetTextFont(42)
        rightaxis.SetTextSize(0.05)
	rightaxis.SetTextColor(rt.kBlue)

	rightaxis.DrawLatex(10500, 9.25e-5, "10^{-1}")
	rightaxis.DrawLatex(10200, 1, "1")
	rightaxis.DrawLatex(10500, 8.74e-4, "0.5") #6.1e-4 with GV
	#rightaxis.DrawLatex(10300, 1.06e-3, "0.7")
	rightaxis.DrawLatex(10500, 3.68e-3, "0.9") #2.02e-3 with GV
	rightaxis.DrawLatex(10500, 8.8160452e-06, "10^{-2}")


	tick_pglobal_1 = rt.TLine(9734, 1, 10000, 1)
	tick_pglobal_1.Draw("same")
	tick_pglobal_0p9 = rt.TLine(9734, 3.68e-3, 10000, 3.68e-3) #2.02e-3 with GV
	tick_pglobal_0p9.Draw("same")
	tick_pglobal_0p5 = rt.TLine(9734, 8.74e-4, 10000, 8.74e-4) #6.1e-4 with GV
	tick_pglobal_0p5.Draw("same")
	tick_pglobal_0p1 = rt.TLine(9734, 9.25e-5, 10000, 9.25e-5)
	tick_pglobal_0p1.Draw("same")
	tick_pglobal_0p01 = rt.TLine(9734, 8.8160452e-06, 10000, 8.8160452e-06)
	tick_pglobal_0p01.Draw("same")
...
#            legThyModel.AddEntry(None,"","")
    ...
    if options.doSignificance:
        ...
        #c.SaveAs(options.outDir+"/signif_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_pvalue.pdf")
        c.SaveAs("Figure_014.pdf")
	#c.SaveAs(options.outDir+"/signif_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_pvalue.png")  
        #c.SaveAs(options.outDir+"/signif_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_pvalue.C")
    else:
        if options.bayes:
            if options.noSys:
