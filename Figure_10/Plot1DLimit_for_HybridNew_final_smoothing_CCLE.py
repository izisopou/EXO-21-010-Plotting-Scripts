...
def setstyle():
    ...
    rt.gStyle.SetPadTopMargin(0.) #0. CCLE
    rt.gStyle.SetPadRightMargin(0.02) #0.02 CCLE
    rt.gStyle.SetPadBottomMargin(0.07) #0.07 CCLE
    rt.gStyle.SetPadLeftMargin(0.) #0.12 CCLE
    ...
    rt.gStyle.SetLabelSize(0.07,"xyz") #0.05
...
if __name__ == '__main__':
    ...
    legendLabel = {'RSGravitonGG':'RS graviton (gg#rightarrowG#rightarrowgg)',
                   ...
                   "DMbb1GeV": "DM mediator to bb",
                    "String": "String",
                    "Diquark":"Diquark: S_{uu} #rightarrow #chi#chi #rightarrow (ug)(ug)",
                   'gg':'gluon-gluon',
                   'qq':'Suu->#chi #chi',
                   'qg':'quark-gluon',
...
#                  expectedLimit_minus2sigma_new[i] = expectedLimit_minus2sigma[(Box,model)][i]
	...
	leg_alpha = rt.TLegend(0.45,0.82,0.85,0.97) #0.4, 0.78, 0.80, 0.93
	...
	leg_alpha.SetTextSize(0.06) #0.065
...
#            legThyModel.AddEntry(None,"","")
    ...
    if 'PF' in Box or options.massMax>1600:
	    ...
	    h_limit.GetXaxis().SetTitleSize(0.075)
	...
	c.SaveAs(options.outDir+"/signif_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+".png")
		...
		c.SaveAs(options.outDir+"/limits_freq_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_HN_above_4500_smoothed_CCLE.png")
                ...
                outFile = rt.TFile.Open(options.outDir+"/limits_freq_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+".root","recreate")
                outFile.cd()
                c.Write()
                for thyModel in thyModelsToDraw: 
