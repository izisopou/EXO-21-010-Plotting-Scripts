...
def getThyXsecDict(alphap):    
    ...
    xsecFiles = ['Diquark_0p%.0f.txt'%(alphap)]
...
if __name__ == '__main__':
    ...
    if options.doSignificance:
        h_limit.SetTitle(" ;Four-jet resonance mass [GeV];Local p-value")
    else:
    ...
    if options.doSignificance:
        #lsig.DrawLatex(0.20,0.77,"m_{#chi}/M_{S} = %.2f, y_{uu}=0.4, y_{#chi}=0.6"%(alphatrue))
            lsig.DrawLatex(0.6,0.85,"Y#rightarrow X X #rightarrow (jj) (jj)") #0.2, 0.77
            lsig.DrawLatex(0.6,0.81,"M(X)/M(Y)= %.2f"%(alphatrue)) #0.2, 0.74
	...
	rightaxis_title.DrawLatex(11200, 0.4, "Global p-value") #11200,0.4 for "Global p-value"
...
#            legThyModel.AddEntry(None,"","")
	...
	#c.SaveAs(options.outDir+"/signif_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_pvalue.png")  
        ...
        c.SaveAs("Figure_13.pdf")
	c.SaveAs("Figure_13.png")  
        c.SaveAs("Figure_13.C")
    else:
        if options.bayes:
            if options.noSys:
