...
void Plot_Combined_inclusive(){

	 double lumi 	     = 137000;    //35900 for 2016, 41850 for 2017, 77750 for 2016+2017
	 double startit	     = 1530;
	 double endit	     = 9067;
	 double startit_RM   = 1530;
	gROOT->LoadMacro("setTDRStyle.C");
	setTDRStyle();
	 //gStyle->SetOptFit(1111);
	  
	double gg_8TeV_obs_limit = 4.5e-5;
        double qq_4TeV_obs_limit = 0.00014;
        double qq_2TeV_obs_limit = 0.00937;
	 //ROOT::Math::MinimizerOptions::SetDefaultMaxFunctionCalls( 20000);   //set the number of parameter searches for convergence

	char *image_name = "_2016_2017_Shapes_tot_err";

	TFile *f_RM= new TFile("HISTOS_2016_2017_2018_M4J_1p5_alpha0p1.root");
	TFile *f_SF_atlas= new TFile("DijetFitResult_PFDijet2017_4J_alphabin1_atlas_incl.root");
        TFile *f_SF_dijet= new TFile("DijetFitResult_PFDijet2017_4J_alphabin1_dijet_incl.root");
        TFile *f_SF_moddijet= new TFile("DijetFitResult_PFDijet2017_4J_alphabin1_moddijet_incl.root");
        TFile *f_MC= new TFile("QCD2017_Standardcuts_M4Jgt1p5_nobreak.root");


TFile *f_gg= new TFile("ResonanceShapes_Suu8600_Chi2150_SR_noslices.root");
...
TFile *f_qq= new TFile("ResonanceShapes_Suu2000_Chi500_SR_noslices.root");
...
TFile *f_qg= new TFile("ResonanceShapes_Suu5000_Chi1250_SR_noslices.root");
...
shape_qg_binned->SetBinContent(i,1000*shape_qg_binned->GetBinContent(i)/((shape_qg_binned->GetBinLowEdge(i+1)-shape_qg_binned->GetBinLowEdge(i))*lumi));
	 ...
	 int    NDF_SF_atlas        =-5;  // 3 parameter function is used for Standard Fit	  
	 double chi_square_SF_dijet = 0;
	 int	NDF_SF_dijet	    =-5;  //     
         double chi_square_SF_moddijet = 0;
	 int	NDF_SF_moddijet	    =-5;  // 
...
double qg	      = shape_qg_binned->GetBinContent(i);
 ...
 if(data_signal>SF_atlas) pull_SF_atlas->SetBinContent(i,(-SF_atlas+data_signal)/errl);
			...
			pull_gg->SetBinContent(i,gg/edata_signal2);
...
cout << "mjj " << mjj << " gg " << gg  << " error " << edata_signal2 << gg/edata_signal2 << " error low " << errl << " error high " << errh << endl;
...
shape_qq_binned->SetLineColor(4);
	shape_qq_binned->GetXaxis()->SetRangeUser(1600,2700);
...
TLegend *leg2 = new TLegend(0.2,0.03,0.5,0.3);
...
leg2->SetTextFont(42);
	...
	Standard_Fit_atlas->GetYaxis()->SetRangeUser(0.00000000015,10000.);
        ...
        Standard_Fit_moddijet->GetYaxis()->SetRangeUser(0.000000000015,10000.);
...
l->DrawLatex(0.23,0.83,"#bf{CMS}"); //for PAPER - ilias
l->SetTextSize(0.055);
...
//
	...
	c1->SaveAs("Figure_015.pdf");

/*TFile *fout;

 fout = new TFile("/eos/user/i/izisopou/4jets/CMSSW_10_2_13/src/CMSDIJET/DijetRootTreeAnalyzer/Fig14_forHEPData.root","RECREATE");
